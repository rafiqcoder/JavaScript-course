var classes = {
  ...
  pre_requisites_react: {
    title: "Pre-Requisites for React",
    url: "50-pre-requisites-react.html",
    section_start: "Section 6 - React Basics",
  },
};
...
document.addEventListener(
  ...
  function () {
    ...
    if (current_class != classes_names[0]) {
      the_prev_url =
        classes[classes_names[classes_names.indexOf(current_class) - 1]].url;
      prev_class.innerHTML =
        "<a href='" + the_prev_url + "'>" + "&lt;&lt; Previous Lesson" + "</a>";
    }

    if (current_class != classes_names[classes_names.length - 1]) {
      the_next_url =
        classes[classes_names[classes_names.indexOf(current_class) + 1]].url;
      next_class.innerHTML =
        "<a href='" + the_next_url + "'>" + "Next Lesson &gt;&gt;" + "</a>";
    }
    ...
    for (a = 0; a < classes_names.length; a++) {
      if (classes[classes_names[a]].section_start) {
        index_element.innerHTML +=
          ...
          classes[classes_names[a]].section_start +
          "</div>";
      }
      ...
      if (current_class != classes_names[a]) {
        index_element.innerHTML +=
          "<li>" +
          "<a href='" +
          classes[classes_names[a]].url +
          "'>" +
          classes[classes_names[a]].title +
          "</a>" +
          "</li>";
      } else {
        index_element.innerHTML +=
          "<li>" + classes[classes_names[a]].title + "</li>";
      }
    }
